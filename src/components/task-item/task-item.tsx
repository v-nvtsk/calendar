...
function renderTags(tags: string): JSX.Element[] {
  ...
  if (tags.length === 0) {
    ...
    result = tags.split(",").map((tag) => (
      <Link to={`/calendar/list?filter=${JSON.stringify({ tagText: tag.trim() })}`} key={tag} className={styles.tag}>
        {tag.trim()}
      </Link>
    ));
  }
  ...
  return (
    <li className={styles.item} data-id={itemData.id}>
      ...
      <div className={styles.titleGroup}>
        <Link to={`../task?id=${itemData.id}`} className={styles.title}>
          {itemData.taskTitle}
        </Link>
      ...
      <div className={styles.btnGroup}>
        <button className="btnEdit" data-id={itemData.id} onClick={callbacks.editHandler}>
          ✏️
        </button>
        <button className="btnDelete" data-id={itemData.id} onClick={callbacks.deleteHandler}>
          🗑️
        </button>
      </div>
      {itemData.tags && <div className={styles.tagsGroup}>{renderTags(itemData.tags)}</div>}
    </li>
  );
}
