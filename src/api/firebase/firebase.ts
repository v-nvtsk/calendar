import { AuthData, AuthRequestBody, FetchOptions, IFirebase, UserData, WithId } from ".";
import { createFetchOptions } from "../../helpers/fetch/fetch-options";
import { APP_PREFIX, type Filter } from "../calendar.types";
...
class Firebase<T> implements IFirebase<T> {
  ...
  public async getUserData(): Promise<UserData | null> {
    ...
    return user.users[0] as UserData;
  }
  ...
  async read(entity: string, filter: Partial<Filter>): Promise<any[] | {}> {
    ...
    if (filter.dateFrom || filter.dateTo) params.orderBy = '"startDate"';
