import { createFetchOptions } from "../../helpers";
import { APP_PREFIX, TodoItem, UpdateTodoItem } from "../calendar.types";
...
describe("firebase", () => {
  ...
  describe("auth", () => {
    ...
    describe("get user data", () => {
      ...
      describe("update Email", () => {
        const newEmail = "test@email.change";
        it("should update user email", async () => {
          await fireStore.signIn(USER_PREFIX, USER_PASSWORD);
          const url = `https://identitytoolkit.googleapis.com/v1/accounts:update?key=${API_KEY}`;
          const options = createFetchOptions("POST", null, {
            idToken: "test_token",
            email: newEmail,
            returnSecureToken: true,
          });

          await fireStore.updateEmail(newEmail);
          expect(global.fetch).toHaveBeenCalled();
          expect(global.fetch).toHaveBeenCalledWith(url, options);
        });
        ...
        it("should return null if request fails", async () => {
          await fireStore.signIn(USER_PREFIX, USER_PASSWORD);
          global.fetch = mockAuthFetch400;
          expect(await fireStore.updateEmail(newEmail)).toBeNull();
          expect(global.fetch).toHaveBeenCalled();
        });
  ...
  describe("fetchEntity", () => {
    it("should try to renew auth on 401 error", async () => {
      global.fetch = jest.fn().mockResolvedValue({
        ok: false,
        status: 401,
      });
      jest.spyOn(fireStore, "renewAuth").mockImplementation = jest.fn();
      await fireStore.read(entity, {});
      expect(fireStore.renewAuth).toHaveBeenCalled();
    });
  });
});
