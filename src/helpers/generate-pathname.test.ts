...
describe("generatePathname", () => {
  it("should return correct path", () => {
    const testData = { newView: "", year: 2024, month: 0, date: 1, week: 1, id: "testId" };
    ...
    expect(generatePathname({ ...testData, newView: "week" })).toBe("/calendar/week?year=2024&week=1");
    expect(generatePathname({ ...testData, newView: "list" })).toBe("/calendar/list?year=2024&month=0&date=1");
    expect(generatePathname({ ...testData, newView: "task" })).toBe("/../task/view?id=testId");
  });
});
