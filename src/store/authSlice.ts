...
export const updateEmail = createAsyncThunk(
  ...
  async (payload: { newEmail: string }, { rejectWithValue }) => {
    try {
      ...
      const result = await firebase.updateEmail(newEmail);
      if (!result) throw new Error("Email update is not possible");
    } catch (e: any) {
      return rejectWithValue(e.message);
    }
    return null;
...
export const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateEmail.pending, (state: AuthState) => {
        const authState = state;
        authState.isLoading = true;
      })
      .addCase(updateEmail.fulfilled, (state: AuthState) => {
        const authState = state;
        authState.isLoading = false;
        authState.errorState = "";
      })
      .addCase(updateEmail.rejected, (state: AuthState, action) => {
        const authState = state;
        authState.isLoading = false;
        authState.errorState = action.payload as string;
      });
