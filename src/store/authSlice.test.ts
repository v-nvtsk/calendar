import { store } from ".";
import { APP_PREFIX } from "../api/calendar.types";
import { AuthData } from "../api/firebase";
import firebase from "../api/firebase/firebase";
import { checkAuth, resetPassword, signIn, signOut, signUp, updateEmail } from "./authSlice";
...
describe("Auth", () => {
  ...
  jest.spyOn(firebase, "updateEmail").mockImplementation((email) => Promise.resolve(email));
  ...
  describe("changeEmail", () => {
    it("should call Firebase updateEmail", async () => {
      const testEmail = "test@test.test";
      await store.dispatch(updateEmail({ newEmail: testEmail }));
      expect(firebase.updateEmail).toHaveBeenCalledWith(testEmail);
      expect(store.getState().auth.errorState).toEqual("");
    });
    it("should set errorState if updateEmail fails", async () => {
      jest.spyOn(firebase, "updateEmail").mockResolvedValue(null);
      await store.dispatch(updateEmail({ newEmail: "test" }));
      expect(store.getState().auth.errorState).toEqual("Email update is not possible");
    });
  });
});
