...
describe("Calendar", () => {
  ...
  describe("read", () => {
    ...
    const item1 = { ...newItem, startDate: new Date("2024-01-01").valueOf(), id: 2 };
    const item2 = { ...newItem, startDate: new Date("2025-01-02").valueOf(), id: 3 };
    const item3 = { ...newItem, startDate: new Date("2023-01-03").valueOf(), id: 1 };
    ...
    it("should read sorted by startDate todo items", async () => {
      expect(store.getState().calendar.items.length).toBe(0);
      jest.spyOn(firebase, "read").mockResolvedValue({
        [item1.id]: item1,
        [item2.id]: item2,
      ...
      await store.dispatch(read({}));
      const { items } = store.getState().calendar;
      expect(items.length).toBe(3);
      expect(items[0].startDate).toBeLessThan(items[1].startDate);
      expect(items[1].startDate).toBeLessThan(items[2].startDate);
    });

    it("should filter on read", async () => {
      expect(store.getState().calendar.items.length).toBe(0);
      jest.spyOn(firebase, "read").mockResolvedValue({
        [item1.id]: item1,
        [item2.id]: item2,
      ...
      await store.dispatch(read({ text: "some text that isn't in items" }));
      expect(store.getState().calendar.items.length).toBe(0);
      await store.dispatch(read({ tagText: "non-existing tag" }));
      expect(store.getState().calendar.items.length).toBe(0);
    });
