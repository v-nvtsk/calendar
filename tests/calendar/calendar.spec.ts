  ...
  testViews.map((view) =>
    test(`calendar view ${view.name}`, async () => {
      ...
      await test.step(`open previous ${view.name}`, async () => {
        ...
        await expect(page).toHaveURL(`${calendarBaseURL}${view.prev}`);
      });
